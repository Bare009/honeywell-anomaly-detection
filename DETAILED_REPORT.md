...
| | |
|---|---|
| **Detection** | PR-AUC **0.938** · Recall @ 1% alert budget **0.896** · Macro-F1 **0.861** (9 classes) |
| **Calibration** | Expected Calibration Error **0.0009** |
| **Latency** | **2.31 ms** median, **3.39 ms** p95 per event, single CPU core |
| **Dataset** | 168,968 synthetic events · 260 entities · 45 days · **0.89% anomalies** |
| **Verification** | **643 automated tests**, all passing · deterministic under seed 42 |
...
detector raises risk to a floor of 90 and overrides the predicted type.
...
inside the alert budget: **autoencoder 0.00, GRU 0.30, classifier 0.70.** The fused score passes
through isotonic calibration (24 knots) so a risk of 70 means roughly a 70% observed attack rate.

Two thresholds answer two different questions — `budget_threshold` 19.05 ("is this in the top 1% I can
...
| Recall @ 1% alert budget | **0.8963** | ≥ 0.80 | PASS |
| Macro-F1 (9 classes) | **0.8607** | ≥ 0.85 | PASS |
| Calibration ECE | **0.0009** | ≤ 0.05 | PASS |
| Latency (median / p95) | **2.31 / 3.39 ms** | < 50 ms | PASS |
| ROC-AUC | 0.9956 | context only | — |

### What the 1% budget means operationally

A 1% budget is **389 events** an analyst reviews. Inside it: **337 of 376 attacks found**, **52 false
positives**, precision **0.866**. So reviewing the top 1% of ranked events surfaces roughly seven in
eight of all attacks in the period.

### Per-class classification

| Class | Precision | Recall | F1 | Support |
|---|---|---|---|---|
| `normal` | 0.9996 | 0.9963 | 0.9980 | 38,499 |
...
`lateral_movement` and `low_and_slow_exfil` reach F1 1.000 — the two classes the naive baseline was
worst at, which is direct evidence the sequence and windowed features earn their place.
`device_spoofing` is the clear weak point (Section 9).

**643 automated tests** cover the domain contracts, generator statistics, feature parity, every model
...
**`device_spoofing` precision is 0.196** — 131 normal events misclassified, the main drag on macro-F1.
The cause is instructive: we deliberately made spoofing detectable *only* from per-entity device
history (an unfamiliar MAC on an ordinary corporate OS) after removing the globally-obvious variant as
a data leak. That made the class realistic and hard, and the model now over-triggers on benign device
churn. This is the clearest target for further work.
...
and detection misses are not charged to it.

**The autoencoder carries zero linear fusion weight.** It still feeds the classifier as an input and
remains the source of per-feature reconstruction explanations, but it does not independently drive
ranking here. Honest reading: on data this well-featurized the supervised classifier subsumes its
detection contribution. Its value would be greater with little or no labelled attack data — the
realistic case, and why we keep it.
...
**The `impossible_travel` detector's type precision is 0.263.** It is reliably right that something is
wrong (0.789 anomaly precision) but a poor type oracle, because impossible travel often co-occurs with
...
| Criterion | Evidence |
|---|---|
| Detection accuracy on imbalanced data | PR-AUC 0.938 at 0.97% prevalence; accuracy deliberately not reported |
| Anomaly-type classification | Macro-F1 0.861 over 9 classes; three at F1 1.000 |
| Low false positives (top 1% budget) | Recall 0.896 within 389 events at precision 0.866 |
...
| Scalable design | Stateless scorer, 2.31 ms median, containerised (wiring gap §9) |
